...
// تنسيق قيمة الميزانية
function formatBudget(value) {
    return '$' + parseInt(value).toLocaleString();
}

// تحديث قيمة الميزانية عند تحريك السلايدر
...
function searchTrips() {
    ...
    if (destination === 'all' || destination === '') {
        // عرض جميع الرحلات ضمن الميزانية
        filteredTrips = tripsData.filter(trip => trip.budget <= budget);
    } else {
        // البحث حسب الوجهة المحددة والميزانية
...
function displayResults(trips) {
    ...
    trips.forEach(trip => {
        ...
        // حساب التوفير إذا كان الميزانية أكبر من تكلفة الرحلة
...
// إنشاء حقل نصي للبحث عن وجهات غير موجودة في القائمة
...
window.addEventListener('load', function() {
    ...
    // تعيين القيمة الافتراضية للميزانية
    budgetValue.textContent = formatBudget(budgetSlider.value);
    
    // عرض بعض الرحلات عشوائياً عند تحميل الصفحة
